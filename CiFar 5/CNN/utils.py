...
##
def feature_model(num_classes, x_train, y_train, epochs = 10, batch_size = 32, verbose = 0):
    
    model = define_model(num_classes)                                                       ## Build model
    model.fit(x_train, y_train, epochs=epochs, batch_size=batch_size, verbose=verbose)      ## Train the model with training data
    feature_model = Model(model.inputs, outputs=model.get_layer('features').output)         ## Create new model that outputs feature layer
    return feature_model  

##

def define_model3(num_classes):
    model = Sequential()
    ...
    model.add(Flatten())
    model.add(Dense(128, activation='relu', kernel_initializer='he_uniform', name = 'features'))
    model.add(BatchNormalization())
...
def feature_model2(num_classes, x_train, y_train, x_test, y_test, epochs = 10, batch_size = 32, verbose = 0):
    model = define_model3(num_classes)                           ## Build model
    model.fit(x_train, y_train, epochs=epochs, validation_split = 0.2, batch_size=batch_size, verbose=verbose)   ## Train the model with training data
